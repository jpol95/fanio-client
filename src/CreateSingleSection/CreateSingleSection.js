...
export default class CreateSingleSection extends React.Component {
  ...
  handleSectionOrder = (e) => {
    ...
    p.then(() =>
      this.props.handleAddSection({
        ...this.state,
        sectionId: this.props.sectionId,
        canSubmit: !this.invalidOrder() && !this.invalidTitle(),
      })
    );
  };
  ...
  render() {
    return (
      <div className="create-section-form">
        <label htmlFor={`section-title-${this.props.id}`}>
          What's the title of this{" "}
          {this.props.type && this.props.type[this.listName]}?
        </label>
        {this.invalidTitle() && (
          <div class="error">
            {this.props.type[this.listName]} title is required
          </div>
        )}
        <input
        ...
        />
        <label htmlFor={`section-order-${this.props.id}`}>
          In what order does this{" "}
          {this.props.type && this.props.type[this.listName]} come?
        </label>
        {this.invalidOrder() && (
          <div class="error">
            {this.props.type[this.listName]} order must be a number above 0
          </div>
        )}
        <input
