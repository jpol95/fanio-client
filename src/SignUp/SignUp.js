...
export default class Signup extends React.Component {
  ...
  handleUsernameChange = (e) => {
    ...
    this.setState({
      ...this.state,
      username: { touched: true, value: username },
    });
  };
  ...
  handlePasswordChange = (e) => {
    ...
    this.setState({
      ...this.state,
      password: { touched: true, value: password },
    });
  };
  ...
  handlePasswordConfirmChange = (e) => {
    ...
    this.setState({
      ...this.state,
      passwordConfirm: { touched: true, value: passwordConfirm },
    });
  };
  ...
  invalidPassword = () => {
    if (this.state.password.value.length < 8) {
      return (
        <div className="error">Password must be longer than 8 characters </div>
      );
    }
    if (this.state.password.value.length > 72) {
      return (
        <div className="error">Password must be less than 72 characters</div>
      );
    }
    ...
    if (
      !this.state.password.value.match(
        /(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&])[\S]+/
      )
    )
      return (
        <div className="error">
          Password must contain a lower case character, an upper case character,
  ...
  handleSubmitUser = (e, personalInfo) => {
    ...
    FetchService.postUser(user).then(() => {
