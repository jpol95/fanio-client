...
export default class CreateSingleInstallment extends React.Component {
  ...
  handleTypeChange = (e) => {
    this.setState(
      ...
      () =>
        this.props.handleAddInstallment({
          ...this.state,
          canSubmit: !this.invalidType() && !this.invalidTitle(),
        })
    );
  };
  ...
  render() {
    return (
      <div className="single-installment-form">
        <label htmlFor={`type-${this.props.installId}`}>
          What type of installment is this?
        </label>
        {this.invalidType() && this.state.type.touched && (
          <div class="error">Installment type is required</div>
        )}
        <select
          onChange={this.handleTypeChange}
          id={`type-${this.props.installId}`}
        >
          <option></option>
          {this.types &&
        ...
        {this.state.type.value !== "" && (
          <React.Fragment>
            <label htmlFor={`name-${this.props.installId}`}>
              What is the name of this {this.state.type.value}?
            </label>
            {this.invalidTitle() && this.state.title.touched && (
              <div class="error">Installment title is required</div>
            )}
