...
export default class EditSection extends React.Component {
  ...
  getInstallment = () => {
    return this.context.installmentList.find(
      (installment) => this.installmentId === installment.id
    );
  };
  getType = () => {
  ...
  getSection = () => {
    return this.context[this.subId ? "subList" : "sectionList"].find(
      (section) => this[this.subId ? "subId" : "sectionId"] === section.id
    );
  };

  handleEditSection = async (e) => {
    e.preventDefault();
    const link = this.subId
      ? `/sections/sub/${this.props.match.params.subId}`
      : `/sections/section/${this.props.match.params.sectionId}`;
    const sectionCopy = {
      title: this.state.title.value,
      order: this.state.order.value,
      id: this.subId ? this.subId : this.sectionId,
    };
  ...
  invalidOrder = () => {
    return (
      ...
      this.state.order.value < 0 ||
      !Number.isInteger(Number(this.state.order.value))
    );
  };
  ...
  render() {
    ...
    const listName = this.props.match.params.subId
      ? this.getType().subName
      : this.getType().sectionName;
    return (
      <form onSubmit={this.handleEditSection}>
        <label htmlFor={`section-title-${this.getSection().id}`}>
        ...
        />
        <label htmlFor={`section-order-${this.getSection().id}`}>
        ...
        <br />
        <button
          disabled={this.invalidOrder() || this.invalidTitle()}
