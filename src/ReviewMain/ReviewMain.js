...
export default class ReviewMain extends React.Component {
  ...
  handleDeleteReview = () => {
    if (this.isLoggedInUser) {
      FetchService.deleteReview(this.reviewId).then(() => {
        const tableName = !!this.subId ? "subList" : "sectionList";
        this.context.handleDeleteReview(this.reviewId, tableName);
        this.props.history.push(
          `/users/${this.userId}/fandoms/${this.fandomId}/installment-view/${this.installmentId}`
        );
      });
    }
  };
          ...
          <>
            ...
            <Link
              ...
              to={`/users/${this.userId}/fandoms/${
                this.fandomId
              }/installments/${this.installmentId}/sections/${this.sectionId}/${
                this.subId ? `subs/${this.subId}/` : ``
              }reviews/${this.reviewId}/edit-review`}
            >
              Edit Review{" "}
            </Link>
            <button
